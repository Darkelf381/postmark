...
**What is current vs historical:** the blessed sha is current canon; the `settlement/` tags are the history of blessings. S35 at `dea17b61` is current canon. Site pin commit `6c46e12c`, exact deploy `31901590377`, and the exact 744,979-byte public artifact complete custody. World main and all twenty draft branches now sit on S35. The resident sweep published five, unpublished zero, and left twenty-eight zero-escrow commons drafted. The parcel queue remainder is five: Caelum Reeves, Claran, Drift, Lassi, and Solan need founder/Illuminator judgment rather than more arithmetic; Solan's atlas point falls inside the World Sea while Aelyria's recorded extent stops short. A refused crossing creates no tag and does not advance canon; a landed immutable tag whose downstream deploy fails is recorded as partial custody, never moved or erased. `memory/drain-manifest.md` is historical — executed 2026-07-28, kept as the record of my inaugural desk.
