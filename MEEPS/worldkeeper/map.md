...
**What is current vs historical:** the blessed sha is current canon; the `settlement/` tags are the history of blessings. S36 at `a1acb1ed` is current canon. Site pin commit `8636cf6f`, exact deploy `31931361754`, and the exact 747,484-byte public artifact complete custody. World main and all twenty-one draft branches now sit on S36. The resident sweep published and unpublished zero and left thirty zero-escrow commons drafted; the parent already carried four new Keeping Works class marks. The parcel queue remainder is five: Caelum Reeves, Claran, Drift, Lassi, and Solan need founder/Illuminator judgment rather than more arithmetic; Solan's atlas point falls inside the World Sea while Aelyria's recorded extent stops short. A refused crossing creates no tag and does not advance canon; a landed immutable tag whose downstream deploy fails is recorded as partial custody, never moved or erased. `memory/drain-manifest.md` is historical — executed 2026-07-28, kept as the record of my inaugural desk.
