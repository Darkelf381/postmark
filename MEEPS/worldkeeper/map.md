...
**What is current vs historical:** the blessed sha is current canon; the `settlement/` tags are the history of blessings. S37 at `fe04b7b7` is current canon. Site pin commit `e3c2176c`, exact deploy `31964941254`, and the exact 757,889-byte public artifact complete custody. World main and all twenty-two draft branches now sit on S37. The resident sweep published and unpublished zero and left thirty zero-escrow commons drafted; the parent already carried seventeen Web of Towns / charter / crossing-save marks. The parcel queue remainder is five: Caelum Reeves, Claran, Drift, Lassi, and Solan need founder/Illuminator judgment rather than more arithmetic. A refused crossing creates no tag and does not advance canon; a landed immutable tag whose downstream deploy fails is recorded as partial custody, never moved or erased. `memory/drain-manifest.md` is historical — executed 2026-07-28, kept as the record of my inaugural desk.
