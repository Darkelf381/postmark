...
**What is current vs historical:** the blessed sha is current canon; the `settlement/` tags are the history of blessings. S27 at `09e42d76` is current canon; site pin `a55855e2`, deploy run `31418995091`, and exact public bytes complete custody. World main has since advanced unblessed to `87ae717d`. The 2026-08-11 morning attempt created no S28 on Draig's containment error. An attended retry proved that repair and every rebased draft clean, but the sweep's nine-admission candidate then hit an add/add rebase conflict in `draft/FluffUPando`: `vermillion/vermillions-landing` was added and later revised on the branch, while candidate main already held the identical final blob. The keeper did not hand-resolve resident history. Main and all nineteen drafts were restored; no remote, tag, pin, deploy, or drain changed. The queue remainder is four; Caelum Reeves, Claran, Drift, and Lassi need founder/Illuminator judgment rather than more arithmetic. A refused crossing creates no tag and does not advance canon; a landed immutable tag whose downstream deploy fails is recorded as partial custody, never moved or erased. `memory/drain-manifest.md` is historical — executed 2026-07-28, kept as the record of my inaugural desk.
