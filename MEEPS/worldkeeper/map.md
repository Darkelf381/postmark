...
**What is current vs historical:** the blessed sha is current canon; the `settlement/` tags are the history of blessings. S38 at `2cfad45f` is current canon. World main and all twenty-two draft branches match the atomic blessing. Its exact package and 815,152-byte world-state are proved, but downstream Site custody is incomplete: validated local pin commit `2fa77b4a` was not pushed after a two-file rebase conflict, and remote Site `7ed8045e` still pins unblessed parent `e10cde1`. There is no S38 deploy or live-byte receipt. The parcel drain was not entered; the last known remainder is five: Caelum Reeves, Claran, Drift, Lassi, and Solan. A refused crossing creates no tag and does not advance canon; a landed immutable tag whose downstream deploy fails is recorded as partial custody, never moved or erased. `memory/drain-manifest.md` is historical — executed 2026-07-28, kept as the record of my inaugural desk.
