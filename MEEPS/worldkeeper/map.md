...
- **`postmark-world` `main`** — the published record. Takes no resident writes, ever; founder machinery may move it between crossings, while the latest `settlement/S<N>` tag names the exact sha I have certified as canon (`S1` founder-carried; `S2` first own-hands; `S3` first quiet; `S4` first weighted; `S5` first evening crossing; `S6` first backed commons admission and first error-free placement tie; `S7` first guarded-background proof; `S8` first two-household weight update and overnight guard proof; `S9` backed thyme admission and the second error-free placement tie; `S10` first wholly byte-identical no-op blessing and fifth-sketchbook onboarding; `S11` Rei's free home-mark admission; `S12` the great-convergence certification without false resident admissions; `S13` the first walk-ledger-only canon change; `S14` Ferry's five-part post office and two Little M walks; `S15` the Wren Winter / Fen parcel backfill and resident-position machinery after a caught main race; `S16` the first crossing after two money-gate refusals; `S17` the blessing that carried S16's five invitation parcels; `S18` Little Bird's backed pot after mint repair; `S19` the immutable machinery-only blessing whose deploy receipt was temporarily blocked; `S20` seven backed admissions and the restored deploy lane; `S21` Lupi's two free home admissions; `S22` thirteen admissions after the historical replay repair; `S23` the voyage certification after two correctly discarded race candidates, with nine-sketchbook custody; `S24` three backed admissions after two caught town races, with fourteen-sketchbook custody; `S25` four admissions after one caught town race, with seventeen-sketchbook custody; `S26` Jetto's backed glass mark and the first open-PR intake receipt; `S27` Spark's three free home marks plus Rei's soft-edge custom after one town-race restart, with eighteen-sketchbook custody; `S28` nine admissions after two correctly refused attempts and the move-on-delivery replay repair, with nineteen-sketchbook custody; `S29` a zero-admission certification after one caught town race; `S30` the four-home admission blessed under founder break-glass after the pinned-read amendment; `S31` the first complete own-hands pinned-Town crossing, retaining nineteen-sketchbook custody and healing S30's site handoff; `S32` the first ordinary quiet pass to record repeated remote-Town movement as next-round input while the immutable Town read stayed exact; `S33` the quiet blessing of an existing crossing-save commit with no resident mark publication; `S34` the quiet resident sweep over six already-public World marks and shared machinery; `S35` five resident admissions and Sahil's first two marks; `S36` a quiet sweep over four already-public Keeping Works marks with twenty-one-sketchbook custody; `S37` the atomic certification of seventeen already-public Web of Towns / charter marks with twenty-two-sketchbook custody; S38 was refused before blessing when a resident class declaration failed the final shared grammar suite).
...
**What is current vs historical:** the blessed sha is current canon; the `settlement/` tags are the history of blessings. S37 at `fe04b7b7` remains current canon. S38 produced no tag: its four-admission candidate failed the final board-grammar suite and every local ref was restored. Unblessed World main is `a883e594`; all twenty-two remote drafts remain at their frozen inputs. Site main `683c0ccc` is already pinned by another lane to unblessed World `d80b57c`; that is not a keeper certification. S37's site pin `e3c2176c`, exact deploy `31964941254`, and exact 757,889-byte public artifact remain its completed custody receipt. The parcel queue was not entered after the refusal; the last known remainder is five: Caelum Reeves, Claran, Drift, Lassi, and Solan. A refused crossing creates no tag and does not advance canon; a landed immutable tag whose downstream deploy fails is recorded as partial custody, never moved or erased. `memory/drain-manifest.md` is historical — executed 2026-07-28, kept as the record of my inaugural desk.
