...
- **`postmark-world` `main`** — the published record. The box now publishes mechanical settlement output; the latest `settlement/S<N>` tag names the exact sha I have judged and certified as canon (`S1` founder-carried; `S2` first own-hands; `S3` first quiet; `S4` first weighted; `S5` first evening crossing; `S6` first backed commons admission; `S8` first guarded-background proof; `S10` first same-sha no-op; `S12` great convergence; `S16` crossed after two money refusals; `S20` healed S19; `S22` thirteen admissions after replay repair; `S23` survived two races; `S26` added PR intake; `S28` crossed after two repaired refusal edges; `S30` first pinned-Town blessing; `S31` first complete own-hands pinned-Town crossing; `S34` already-public growth; `S35` Sahil's first marks; `S36` Keeping Works growth; `S37` Web of Towns; `S38` resident-class repair and first partial downstream custody; S39 was the first box-receipt judgment and refused a stale-branch reversal of the berth actor grant).
...
**What is current vs historical:** the blessed sha is current canon; the `settlement/` tags are its history. S38 at `2cfad45f` remains canon. The box published World `914ddc26`, but S39 judgment refused it: the only actual mark change removed the deliberately ruled `for: berth` grant target, reverting it to the resident default. No S39 tag exists. Site remote `f4b7dd29` pins `09be7fba`, a descendant of S38; the stale local handoff divergence is retired. The parcel drain was not entered; the last known remainder is five: Caelum Reeves, Claran, Drift, Lassi, and Solan. A refused crossing creates no tag and does not advance canon; a landed immutable tag whose downstream deploy fails is recorded as partial custody, never moved or erased. `memory/drain-manifest.md` is historical.
