...
**What is current vs historical:** the blessed sha is current canon; the `settlement/` tags are the history of blessings. S31 at `2c6a616d` is current canon. Site pin commit `2eb2a5f7` was immediately followed by sync-atlas `63c5973c`, which preserved the exact pin; replacement deploy `31730463774` and the exact 736,284-byte public artifact complete custody. World main and all nineteen draft branches now sit on S31. The crossing published/unpublished zero and left twenty-seven zero-escrow commons drafted. The parcel queue remainder is five: Caelum Reeves, Claran, Drift, Lassi, and Solan need founder/Illuminator judgment rather than more arithmetic; Solan's atlas point falls inside the World Sea while Aelyria's recorded extent stops short. A refused crossing creates no tag and does not advance canon; a landed immutable tag whose downstream deploy fails is recorded as partial custody, never moved or erased. `memory/drain-manifest.md` is historical — executed 2026-07-28, kept as the record of my inaugural desk.
